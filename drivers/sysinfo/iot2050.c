...
static int sysinfo_iot2050_get_str(struct udevice *dev, int id, size_t size, char *val)
{
	...
	log_err("\n---Entering sysinfo_iot2050_get_str: id: %d\n", id);
	...
	case BOARD_MAC_ADDR_8:
		log_err("---Before memcpy id: %d\n", id);
		// memcpy(val, priv->info->mac_addr[id - BOARD_MAC_ADDR_START],
		//        ARP_HLEN);
		// for (int i = 0; i < ARP_HLEN; i++) {
		// 	val[i] = priv->info->mac_addr[id - BOARD_MAC_ADDR_START][i];
		// }
		log_err("---after memcpy id: %d\n", id);
		return 0;
	default:
		return -EINVAL;
	};
...
static int sysinfo_iot2050_get_int(struct udevice *dev, int id, int *val)
{
	...
	log_err("---Entering sysinfo_iot2050_get_int: id: %d\n", id);

	switch (id) {
	case BOARD_MAC_ADDR_CNT:
		*val = priv->info->mac_addr_cnt;
		log_err("---Casing sysinfo_iot2050_get_int: val: %d\n", *val);
		return 0;
	default:
		return -EINVAL;
	};
...
static int sysinfo_iot2050_probe(struct udevice *dev)
{
	...
	log_err("---Entering sysinfo_iot2050_probe\n");

	offset = dev_read_u32_default(dev, "offset",
				      TI_SRAM_SCRATCH_BOARD_EEPROM_START);
	priv->info = (struct iot2050_info *)(offset);

	log_err("---Exiting sysinfo_iot2050_probe, info: 0x%08x\n", priv->info);
